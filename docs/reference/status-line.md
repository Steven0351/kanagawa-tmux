...
The tmux status line modules are set as variables and prefixed with `@kanagawa_status_<module>`.
...
set -g status-right "#{E:@kanagawa_status_application}#{E:@kanagawa_status_session}"
set -g status-left ""
```
...
```sh
set -g @kanagawa_[module_name]_icon "icon"
```

### Override the specific module color

```sh
set -g @kanagawa_[module_name]_color "color"
```

### Override the specific module text

```sh
set -g @kanagawa_[module_name]_text "text"
```

### Override the specific module's background color

```sh
set -g @kanagawa_status_[module_name]_bg_color "#{@thm_bg_alt}"
```

### Removing a specific module option

```sh
set -g @kanagawa_[module_name]_[option] ""
```
...
set -g status-right "#{E:@kanagawa_status_[module_name]}"
set -ag status-right "#{E:@kanagawa_status_[module_name]}"
set -agF status-right "#{E:@kanagawa_status_[module_name]}"
...
set -agF status-right "#{E:@kanagawa_status_battery}"
...
set -agF status-right "#{E:@kanagawa_status_cpu}"
...
set -agF status-right "#{E:@kanagawa_status_weather}"
...
set -agF status-right "#{E:@kanagawa_status_clima}"
...
set -agF status-right "#{E:@kanagawa_status_load}"
```
...
set -agF status-right "#{@kanagawa_status_gitmux}"
```
...
tmux:
  styles:
    ...
    untracked: "#[fg=#{@thm_dark_purple},bold]"
    stashed: "#[fg=#{@thm_dark_blue},bold]"
    clean: "#[fg=#{@thm_white},bold]"
...
set -agF status-right "#{E:@kanagawa_status_pomodoro_plus}"
...
```sh
set -g @kanagawa_kube_context_color "#{@thm_red}"
set -g @kanagawa_kube_namespace_color "#{@thm_light_blue}"

run ~/.config/tmux/plugins/catppuccin/tmux/catppuccin.tmux

set -agF status-right "#{E:@kanagawa_status_kube}"
